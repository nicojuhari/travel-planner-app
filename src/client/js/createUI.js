...
export const createNewTripLocationBlock = (allNewTrip) => {
    ...
    for(let oneDestination of allNewTrip) {
        
        

        content += `<div class="trip__destination">
                        <div class="loc__image" ${ oneDestination.image != false ? 'style="background-image: url(' + oneDestination.image + ')"' : ``}></div>
                        <div class="loc__info">
                            <h2 class="loc__name">
                                <span class="loc__city">${oneDestination.destination.city}</span>,
                                <span class="loc__country">${oneDestination.destination.country}</span>
                            </h2>
                            <div class="loc__dates">
                                <div class="loc__date">
                                    <div class="start-end-date">
                                        <span class="date-start">${oneDestination.dateStart}</span>
                                        -
                                        <span class="date-end">${oneDestination.dateEnd}</span>
                                        /
                                        <span class="total-days">${daysDifference(oneDestination.dateStart, oneDestination.dateEnd)} days</span>
                                    </div>
                                </div>
                                <div class="loc__remain">
                                    The Trip will start in: <span class="remain-days">${daysDifference(new Date(), oneDestination.dateStart)}</span> days
                                </div>
                            </div>
                            <div class="loc__weather weather">
                                <div class="weather__title">The weather will be:</div>
                                <div class="weather__content">
                                    <div class="weather__icon">
                                        <img src="https://darksky.net/images/weather-icons/${oneDestination.weather.icon}.png"/>
                                    </div>
                                    <div class="weather__info">
                                        <div class="weather__data"><span>${fahrenheitToCelsius(oneDestination.weather.temperatureMax)}</span> &deg;C</div>
                                        <div class="weather__commnents">${oneDestination.weather.summary}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    for(let trip of trips) {
        ...
        content += `<div class="trip-container" data-trip-nr="${itr}">
                    <div class="trip-container__header">
                        <h4 class="trip-container__title">This trip has <span class="num-icon">${trip.length}</span> destination${trip.length > 1 ? 's': ''}</h4>
